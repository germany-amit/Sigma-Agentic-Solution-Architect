# End-to-End Architecture & Integration MVP
# This single-file application simulates a wide range of skills to meet a
# comprehensive job description for a solutions architect. It uses only
# free and open-source components and is designed to run on Streamlit Cloud.
#
# Key Skills Demonstrated:
# - E2E Architecture & Design (simulated)
# - Data & Process Flows
# - AWS Application & SAP Integration (simulated)
# - Generative AI Solutions (simulated)
# - API Development & Gateway (simulated)
# - Enterprise Architecture & Security Compliance
#
# Requirements: streamlit, numpy, pandas, matplotlib, typing, pydantic
#
...
def sign_payload(secret: str, payload: str) -> str:
    """Simulates signing a payload using HMAC-SHA256."""
...
def verify_jwt(token: str, secret: str) -> bool:
    """Simulates verifying a JWT token."""
    try:
...
with B:
    ...
    # Tiny catalog
    entities = [
        Customer(customer_id="C001", email="a@x.com", segment="Pro", country="IN"),
    ...
    st.json([e.model_dump() for e in entities])
...
with C:
    ...
    with col2:
        ...
        if st.button("Validate", key="validate"):
            ...
            try:
                ok_sig = inbound_sig == sign_payload(secret, json.dumps(json.loads(inbound_body), separators=(',', ':')))
            except Exception:
                ok_sig = False
            st.metric("JWT valid", ok_jwt)
            st.metric("Signature valid", ok_sig)
